...
dplyr::mutate(
  ...
  foto = glue::glue("<center><img src='{url_foto}' width='100' style='border-radius: 50%;'><br><b>{nome}</b> <br> {icon_site} {icon_github} {icon_twitter} {icon_facebook}</center>")
    ) %>% 
...
#| echo: FALSE
df_profs %>% 
  dplyr::filter(atividade == "Monitora") %>% 
  dplyr::select(-atividade) %>% 
  reactable::reactable(columns = list(
        foto = reactable::colDef(html = TRUE, name = "Monitoras", maxWidth = 250), 
        destaque = reactable::colDef(html = TRUE, name = "")
    ))
```
