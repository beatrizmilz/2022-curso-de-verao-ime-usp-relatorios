...
- Para cada aula, teremos um formulário (Google Forms) que deverá ser respondido após as aulas. A maioria das perguntas são relacionadas ao acompanhamento do conteúdo e configuração das ferramentas necessárias. Alguns formulários envolverão o envio de tarefas.

- Teremos um trabalho final, que deverá ser entregue até .colorido[data a ser definida].
...
tibble::tibble(
  ...
  tibble::add_row(
    forma_de_citar = "[@R-tidyverse; @tidyverse2019]",
    resultado = "(Wickham 2021; Wickham et al. 2019)"
  )|> 
  tibble::add_row(
    forma_de_citar = "Segundo Wickham et al. [-@tidyverse2019]",
    resultado = "Segundo Wickham et al. (2019) "
  ) |> 
  knitr::kable()
```
...
- Usamos o rótulo do chunk (ou nome) onde a tabela ou imagem foi criada para fazer a referência cruzada. 
  - Ex: `\@ref(fig:rotulo-da-imagem)` ou `\@ref(tab:rotulo-da-tabela)`.
